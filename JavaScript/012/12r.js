...
function resetScore(){
  const messageParagraphElement = document.querySelector('.js-message');
  messageParagraphElement.innerHTML = `Are you sure you want to reset the score? 
  <button class="js-yes-confirm-button">Yes</button>
  <button class="js-no-confirm-button">No</button>`;

  document.querySelector('.js-yes-confirm-button').addEventListener('click', () => {
    score.wins = 0;
    score.ties = 0;
    score.losses = 0;
    updateScoreElement();
  ...
  document.querySelector('.js-no-confirm-button').addEventListener('click', () => messageParagraphElement.innerHTML = '');
}